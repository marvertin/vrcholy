...
class  (Num a, Ord a, Show a) => Numd a  where
...
instance (Integral a, Show a) => Numd (Ratio a) where
    divd = (/)
    floord = fromIntegral . floor
...
mergeNodes fceLeftNode fceRightNode fceSamePoints = mergeNodes'
     where              
        mergeNodes' _ EmpNd node = trace "levy emp" $ fceRightNode node
        mergeNodes' _ node EmpNd = trace "pravy emp" $fceLeftNode node
        ...
        mergeNodes' rct@(Rect (x1,y1) (x2,y2)) (Node jz1 jv1 sz1 sv1) (Node jz2 jv2 sz2 sv2) = 
            trace ("baleni " ++ show rct) $ packNode $ Node { 
                jz = trace "OOOOO" $ mergeNodes' (Rect (x1, y1) (xmid, ymid)) (jz1) (jz2),
                jv = trace "OOOOO" $mergeNodes' (Rect (xmid, y1) (x2, ymid)) (jv1) (jv2),
                sz = trace "OOOOO" $mergeNodes' (Rect (x1, ymid) (xmid, y2)) (sz1) (sz2),
                sv = trace "OOOOO" $mergeNodes' (Rect (xmid, ymid) (x2, y2)) (sv1) (sv2)
            }
            where xmid = trace "x-----------------x" $ div2 (x1 + x2)
                  ymid = trace "y-----------------y" $ div2 (y1 + y2)
